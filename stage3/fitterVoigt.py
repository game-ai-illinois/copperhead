    ...
    def create_workspace(self, mode, binned=False, category = "All"):
        ...
        if mode == "Z":
            ...
            if ("calib_cat3" in self.label) or ("calib_cat7" in self.label)  or  ("calib_cat21" in self.label):
                ...
                mh_ggh = rt.RooRealVar(
                    "mh_ggh", "mh_ggh", 80.5, 100.8
                )
                self.fitranges["low_Z"] = 80.5
                self.fitranges["high_Z"] = 100.1
                ...
                self.fitranges["low_Z"] = 82.5
                self.fitranges["high_Z"] = 99.5
            elif ("calib_cat24" in self.label)  or ("calib_cat13" in self.label):
                mh_ggh = rt.RooRealVar(
            ...
            elif ("calib_cat18" in self.label) or ("calib_cat20" in self.label)or ("calib_cat28" in self.label) or ("calib_cat27" in self.label):
                mh_ggh = rt.RooRealVar(
            ...
            elif ("calib_cat11" in self.label) or ("calib_cat17" in self.label)  or ("calib_cat29" in self.label):
                mh_ggh = rt.RooRealVar(
